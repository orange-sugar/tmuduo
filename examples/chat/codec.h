...
class LengthHeaderCodec : noncopyable {
 public:
  using StringMessageCallback = std::function<void(const tmuduo::net::TcpConnectionPtr&,
                                                   const std::string& message, Timestamp)>;
  explicit LengthHeaderCodec(StringMessageCallback cb) : messageCallback_(std::move(cb)) {
  }

  void onMessage(const tmuduo::net::TcpConnectionPtr& conn, tmuduo::net::Buffer* buf,
