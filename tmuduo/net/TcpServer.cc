...
TcpServer::TcpServer(EventLoop* loop, InetAddress listenAddr, std::string nameArg, Option option)
    : loop_(loop), hostport_(listenAddr.toIpPort()), name_(nameArg),
      acceptor_(new Acceptor(loop, listenAddr, option == Option::kReusePort)),
      threadPool_(new EventLoopThreadPool(loop)), connectionCallback_(defaultConnectionCallback),
...
TcpServer::~TcpServer() {
  ...
  for (auto& item : connections_) {
    ...
    item.second.reset();
    connPtr->getLoop()->runInLoop(std::bind(&TcpConnection::connectionDestroyed, connPtr));
  }
}
