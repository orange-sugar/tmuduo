...
void tmuduo::net::defaultConnectionCallback(const TcpConnectionPtr& conn) {
  LOG_TRACE << conn->localAddress().toIpPort() << " -> " << conn->peerAddress().toIpPort() << " is "
            << (conn->connected() ? "UP" : "DOWN");
}

void tmuduo::net::defaultMessageCallback(const TcpConnectionPtr& conn, Buffer* buf, Timestamp) {
  buf->retrieveAll();
}

TcpConnection::TcpConnection(EventLoop* loop, std::string name, int sockfd, InetAddress localAddr,
                             InetAddress peerAddr)
    : loop_(loop), name_(std::move(name)), state_(StateE::kConnecting), socket_(new Socket(sockfd)),
      channel_(new Channel(loop, sockfd)), localAddress_(std::move(localAddr)),
      peerAddress_(std::move(peerAddr)), highWaterMark_(64 * 1024 * 1024) {
...
void TcpConnection::send(const void* message, size_t len) {
  if (state_ == StateE::kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([&] { sendInLoop(data); });
    }
  }
}
...
void TcpConnection::send(Buffer&& message) {
  if (state_ == StateE::kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([&, _message = std::move(message)]() mutable {
        sendInLoop(_message.retrieveAllAsString());
      });
    }
  }
}
